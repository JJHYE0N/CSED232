...
public:
    ...
    Vector();//기본 생성자. 모든 멤버 변수를 0 혹은 NULL로 초기화한다.
    ...
    void push_back(const float& value);//데이터의 맨 뒤에 새로운 데이터 value를 추가한다. 2.문제 세부 조건 규칙에 따라 메모리를 동적 할당한다.
    void pop_back();//맨 마지막 데이터를 삭제한다. 이때 메모리 할당은 유지한다.
    void insert(iterator position, const float& value);//데이터의 특정 위치(position)에 value 를 삽입한다.
    // 2.문제 세부 조건 규칙에 따라 메모리를 동적 할당한다.
    void insert(iterator position, const float* first, const float* last);//데이터에서 특정 위치(position)에 float array 를 삽입한다.
